...
import (
	...
	rb "github.com/rumpl/rb/pkg/agent"
	"github.com/rumpl/rb/pkg/runtime"
	"github.com/rumpl/rb/pkg/session"
	"github.com/rumpl/rb/pkg/team"
)

// newRBAdapter creates a new ADK agent adapter from a rb team and agent name
func newRBAdapter(t *team.Team, agentName string) (agent.Agent, error) {
	...
	return agent.New(agent.Config{
		...
		Run: func(ctx agent.InvocationContext) iter.Seq2[*adksession.Event, error] {
			return runRB(ctx, t, agentName, a)
		},
	})
}

// runRB executes a rb agent and returns ADK session events
func runRB(ctx agent.InvocationContext, t *team.Team, agentName string, a *rb.Agent) iter.Seq2[*adksession.Event, error] {
	return func(yield func(*adksession.Event, error) bool) {
		...
		// Convert rb events to ADK events and yield them
